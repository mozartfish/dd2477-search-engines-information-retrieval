...
public class SearchGUI extends JFrame {
  ...
  void init() {
          ...
          public void actionPerformed(ActionEvent e) {
            ...
            if (box != null) {
              ...
              for (int i = 0; i < box.length; i++) {
                if (box[i] != null) relevant[i] = box[i].isSelected();
              }
    ...
    queryWindow.registerKeyboardAction(
        search, "", KeyStroke.getKeyStroke("ENTER"), JComponent.WHEN_FOCUSED);
    ...
    Action setNumberOfWordsNormalization =
        new AbstractAction() {
          public void actionPerformed(ActionEvent e) {
    ...
    Action setEuclideanNormalization =
        new AbstractAction() {
          public void actionPerformed(ActionEvent e) {
   ...
   *
   * @param maxResultsToDisplay The results list is cut off after this many results have been
   *     displayed.
  ...
  void displayResults(int maxResultsToDisplay, double elapsedTime) {
    displayInfoText(
        String.format(
            "Found %d matching document(s) in %.3f seconds", results.size(), elapsedTime));
    box = new JCheckBox[maxResultsToDisplay];
    int i;
    for (i = 0; i < results.size() && i < maxResultsToDisplay; i++) {
      String description =
          i + ". " + displayableFileName(engine.index.docNames.get(results.get(i).docID));
