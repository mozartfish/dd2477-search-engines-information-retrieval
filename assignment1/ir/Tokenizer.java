...
public class Tokenizer {
  ...
  /** This flag should be set to 'true' if all letters should be turned into lowercase. */
  public boolean case_folding = true;

  /** This flag should be set to 'true' if all diacritics (accents etc.) should be removed. */
  public boolean remove_diacritics = true;

  /** This flag should be set to 'true' if all punctuation (full stops etc.) should be removed. */
  public boolean remove_punctuation = true;

  /**
   * The size of the buffer should be considerably larger than the anticipated length of the longest
   * token.
   */
  ...
  /** Characters are read @code{BUFFER_LENGTH} characters at a time into @code{buf}. */
  ...
  public static final char[] SPECIAL_CHAR_MAPPING = {
    'A', 'A', 'A', 'A', 'A', 'A', 'E', 'C', 'E', 'E', 'E', 'E', 'I', 'I', 'I', 'I', 'D', 'N', 'O',
    'O', 'O', 'O', 'O', '*', 'O', 'U', 'U', 'U', 'U', 'Y', 'T', 'S', 'a', 'a', 'a', 'a', 'a', 'a',
    'e', 'c', 'e', 'e', 'e', 'e', 'i', 'i', 'i', 'i', 'd', 'n', 'o', 'o', 'o', 'o', 'o', '/', 'o',
    'u', 'u', 'u', 'u', 'y', 't', 'y', 'A', 'a', 'A', 'a', 'A', 'a', 'C', 'c', 'C', 'c', 'C', 'c',
    'C', 'c', 'D', 'd', 'E', 'e', 'E', 'e', 'E', 'e', 'E', 'e', 'E', 'e', 'G', 'g', 'G', 'g', 'G',
    'g', 'G', 'g', 'H', 'h', 'H', 'h', 'I', 'i', 'I', 'i', 'I', 'i', 'I', 'i', 'I', 'i', 'J', 'j',
    'J', 'j', 'K', 'k', 'k', 'L', 'l', 'L', 'l', 'L', 'l', 'L', 'l', 'L', 'l', 'N', 'n', 'N', 'n',
    'N', 'n', 'n', 'N', 'n', 'O', 'o', 'O', 'o', 'O', 'o', 'O', 'o', 'R', 'r', 'R', 'r', 'R', 'r',
    'S', 's', 'S', 's', 'S', 's', 'S', 's', 'T', 't', 'T', 't', 'T', 't', 'U', 'u', 'U', 'u', 'U',
    'u', 'U', 'u', 'U', 'u', 'U', 'u', 'W', 'w', 'Y', 'y', 'Y', 'Z', 'z', 'Z', 'z', 'Z', 'z'
  };
   ...
   * @param case_folding Should be set to <code>true</code> if every character should be translated
   *     into its lowercase counterpart.
   * @param remove_diacritics Should be set to <code>true</code> if diacritics should be removed
   *     (e.g. é will be e).
   * @param remove_punctuation Should be set to <code>true</code> if punctuation should be removed
   *     (useful in some applications).
   * @param pattern_file The name of the file containing regular expressions for non-standard words
   *     (like dates, mail addresses, etc.).
   */
  public Tokenizer(
      Reader reader,
      boolean case_folding,
      boolean remove_diacritics,
      boolean remove_punctuation,
      String pattern_file) {
    this.reader = reader;
  ...
  /**
   * Normalizes letters by converting to lower-case and possibly removing diacritics. This method is
   * also used for checking whether a character can occur in a token or not.
   *
   * @return code{true} if the (normalized counterpart of the) character can occur within a token,
  ...
  /**
   * @return the @code{true} if there are more tokens to be read, and @code{false} otherwise.
   */
  public boolean hasMoreTokens() throws IOException {
    ...
    if (token_queue.size() == 0) return readTokens();
    else return true;
  }

  /**
   * @return a String containing the next token, or @code{null} if there are no more tokens.
   */
  public String nextToken() throws IOException {
    if (token_queue.size() == 0) {
      if (readTokens()) return token_queue.remove(0);
      else return null;
    } else {
  ...
  /** Adds standard tokens (i.e. tokens not matching any regular expression) to the queue. */
