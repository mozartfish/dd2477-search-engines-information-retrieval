...
public class TokenTest {
  ...
  /**
   * Tokenizes and indexes the file @code{f}. If @code{f} is a directory, all its files and
    ...
    if (f.canRead()) {
      if (f.isDirectory()) {
        ...
        try {
          ...
          Tokenizer tok =
              new Tokenizer(
                  reader, case_folding, remove_diacritics, remove_punctuation, patternsfile);
          int offset = 0;
  ...
  static void printHelpMessage() {
    ...
    System.err.println(
        "  -p <filename> : name of the file containing regular expressions for non-standard words (optional)");
